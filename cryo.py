...
class LaserScan(object):
    ...
    def fit_smooth_freq_auto_parameter(self, x, y,a=None,b=None,c=None,fix_a = False, fix_b=False,fix_c=False,figure=True):
        '''
    ...
    def fit_smooth_freq_batch(self,frequencies,fix_a = False, fix_b=False,fix_c=False,figure=True):
        ...
        for i in range(num):
            ...
            freq = frequencies[i]
            parameters, fit = self.fit_smooth_freq_auto_parameter(index,freq,a=None,b=None,c=None,fix_a = False, fix_b=False,fix_c=False,figure=True)
    ...
    def fit_lorentzian(self, x,y,gamma,a,x0,y0,fix_gamma = False, fix_a=False,fix_x0=False,fix_y0=False,Hz=True,figure=True):
        '''
        x - list of x values
        y - list of y values
        ...
        x0_init=x0
        # Fitfunc
        function = lambda x, gamma, a, x0,y0: a*(0.5*gamma)**2/(np.pi*((x-x0)**2+(0.5*gamma)**2))+y0
    ...
    def fit_lorentzian_auto_parameter(self, x, y,gamma=None,a=None,x0=None,y0=None,fix_gamma = False, fix_a=False,fix_x0=False,fix_y0=False,Hz=True,figure=True):
        '''
        function first finds starting parameters
        '''
        ...
        if y0 is None:
            y0  = y0_new
        parameters, fit = self.fit_lorentzian(x,y,gamma,a,x0,y0,fix_gamma = False, fix_a=False,fix_x0=False,fix_y0=False,Hz=True,figure=figure)

        return parameters, fit
    
    def fit_lorentzian_batch_old(self,data_freq, data_intensity,gamma, a, x0, y0, freq_range=[0, 1e15], fix_gamma=False,
    ...
    def fit_lorentzian_batch(self,data_freq,data_intensity, gamma=None,a=None,x0=None,y0=None,freq_range=[0,1e15],fix_gamma = False, fix_a=False,fix_x0=False,fix_y0=False,Hz=True,figure=True):
        '''
        data_ - list Frequencies,'intensities
        p -list of start parameters 
        gamma - FWHM (sigma)
        ...
        for i in range(len(data_freq)):
            ...
            high_idx=len(freq)
            if i==0:
                param, fits=self.fit_lorentzian_auto_parameter(data_freq[i].iloc[low_idx:high_idx],data_intensity[i].iloc[low_idx:high_idx], gamma=gamma, a=a,x0=x0,y0=y0,fix_gamma=fix_gamma,fix_a=fix_a,fix_x0=fix_x0,fix_y0=fix_y0,figure=False)
                parameters = param
            else:
                param, fits=self.fit_lorentzian_auto_parameter(data_freq[i].iloc[low_idx:high_idx],data_intensity[i].iloc[low_idx:high_idx], gamma=gamma, a=a,x0=x0,y0=y0,fix_gamma=fix_gamma,fix_a=fix_a,fix_x0=fix_x0,fix_y0=fix_y0,figure=False)
                parameters = parameters.append(param)
            ...
            if figure:
                c1=next(color1)
                c2=next(color2)
                ax.scatter(data_freq[i].iloc[low_idx:high_idx]/1e12,data_intensity[i].iloc[low_idx:high_idx]/1000, color=c1)
                ax.plot(data_freq[i].iloc[low_idx:high_idx]/1e12,data_fits[i]/1000, color=c2)
    ...
    def fit_saturation(self, data_power, data_intensity, data_FWHM, R_inf, Is,y0,v0,fix_R_inf=False,fix_Is=False,fix_y0=False,fix_v0=False,Hz=False,nW=True,figure=False):
        '''
    ...
    def fit_saturation_auto_parameter(self, data_power, data_intensity, data_FWHM, R_inf=None, Is=None, y0=None, v0=None, fix_R_inf=False, fix_Is=False,
        ...
        if v0 is None:
            v0 = v0_new
        parameters, fit_intensity, fit_FWHM = self.fit_saturation(data_power, data_intensity, data_FWHM, R_inf, Is, y0, v0, fix_R_inf=fix_R_inf,fix_Is=fix_Is,fix_y0=fix_y0,fix_v0=fix_v0, Hz=Hz, nW=nW, figure=figure)
    ...
    def get_all_power_spectra(self,intensity='APD_1',intensity2='APD_2',power = 'Voltage_Input_3', rep=0, figure = True):
        ...
        for angle in waveplate_angles.tolist():
            ...
            intensities.append(data[intensity].loc[idx_0 + index_min:idx_0 + index_max]+data[intensity2].loc[idx_0 + index_min:idx_0 + index_max])
    ...
    def get_all_iteration_spectra(self,intensity='APD_1',intensity2='APD_2',power = 'Voltage_Input_3', intensity_num=2, rep=0, figure = True):
        data = self.data
        ...
        for num in iteration.tolist():
            ...
            if i==0:
                ...
                if intensity_num==2:
                    intensities = np.array(data[intensity].loc[idx_0 + index_min:idx_0 + index_max]+data[intensity2].loc[idx_0 + index_min:idx_0 + index_max])
                else:
                ...
                if intensity_num==2:
                    intensities = np.append(intensities,np.array(data[intensity].loc[idx_0 + index_min:idx_0 + index_max]+data[intensity2].loc[idx_0 + index_min:idx_0 + index_max]))
                else:
                    intensities = np.append(intensities,np.array(data[intensity].loc[idx_0 + index_min:idx_0 + index_max]))
    ...
    def perform_power_analysis(self,gamma=None,a=None,x0=None,y0=None,freq_range=[0,1e15],fix_gamma = False, fix_a=False,fix_x0=False,fix_y0=False, intensity_monitor='APD_1',power_monitor = 'Voltage_Input_3', rep=0, smooth_freq=False,figure = True, figure_sat = True):
        
        frequencies, intensities, angles, power = self.get_all_power_spectra(intensity = intensity_monitor, power = power_monitor, rep=rep, figure = figure)
        if smooth_freq:
            frequencies = self.fit_smooth_freq_batch(frequencies,fix_a = False, fix_b=False,fix_c=False,figure=False)
        data_fits,data_frequencies, parameters = self.fit_lorentzian_batch(frequencies,intensities, gamma=gamma,a=a,x0=x0,y0=y0,freq_range=freq_range,fix_gamma = fix_gamma, fix_a=fix_a,fix_x0=fix_x0,fix_y0=fix_y0,Hz=True,figure=figure)
